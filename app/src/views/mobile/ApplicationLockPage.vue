<template>
    <f7-page>
        ...
        <f7-list strong inset dividers class="margin-top-half">
            ...
            <f7-list-item v-if="isEnableApplicationLock && isBiometricAvailable">
                <template #after-title>
                    {{ biometricType === 'FaceID' ? 'Unlock with Face ID' : 'Unlock with Fingerprint' }}
                </template>
                <template #after>
                    <f7-toggle :checked="isEnableApplicationLockBiometric" @toggle:change="toggleBiometricAuth($event)"></f7-toggle>
                </template>
            </f7-list-item>
...
const isEnableApplicationLockBiometric = ref<boolean>((settingsStore.appSettings['applicationLockBiometric'] as boolean) || false);
...
function toggleBiometricAuth(enable: boolean): void {
    if (enable) {
        authenticateWithBiometric({
            ...
            if (authenticated) {
                isEnableApplicationLockBiometric.value = true;
                settingsStore.setApplicationLockBiometric(true);
                showToast('Biometric authentication enabled');
            }
        });
    } else {
        isEnableApplicationLockBiometric.value = false;
        settingsStore.setApplicationLockBiometric(false);
