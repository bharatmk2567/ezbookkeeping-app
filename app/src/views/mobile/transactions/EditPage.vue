...
import { isRunningInCapacitor, capturePhoto, triggerHapticImpact } from '@/lib/capacitor.ts';
import {
    ...
    if (isRunningInCapacitor()) {
        capturePhoto().then(result => {
            if (result) {
                ...
                const file = new File([blob], `photo.${result.format}`, { type: `image/${result.format}` });
                uploadPicture(file);
