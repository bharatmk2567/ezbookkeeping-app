<template>
    <f7-page no-navbar no-swipeback login-screen hide-toolbar-on-scroll class="login-page unlock-page">
        ...
        <f7-list form class="login-page-form">
            ...
            <f7-list-button class="login-page-secondary-action margin-top margin-horizontal" v-if="isBiometricAvailable"
                            :text="biometricType === 'FaceID' ? 'Unlock with Face ID' : 'Unlock with Fingerprint'" @click="attemptBiometricUnlock"></f7-list-button>
            <f7-list-button class="login-page-secondary-action margin-top margin-horizontal" v-if="isWebAuthnAvailable && !isBiometricAvailable"
...
const isBiometricAvailable = ref<boolean>(false);
const biometricType = ref<string>('none');

if (isRunningInCapacitor() && (settingsStore.appSettings['applicationLockBiometric'] as boolean)) {
    checkBiometricAvailability().then(result => {
        isBiometricAvailable.value = result.available;
        biometricType.value = result.biometryType;

        if (result.available) {
            authenticateWithBiometric({
                reason: 'Authenticate to unlock ezBookkeeping',
                title: 'Unlock ezBookkeeping',
                subtitle: result.biometryType === 'FaceID' ? 'Use Face ID to unlock' : 'Use your fingerprint to unlock',
            }).then(authenticated => {
                if (authenticated) {
...
function attemptBiometricUnlock(): void {
    if (!isRunningInCapacitor()) return;

    authenticateWithBiometric({
        reason: 'Authenticate to unlock ezBookkeeping',
        title: 'Unlock ezBookkeeping',
        subtitle: biometricType.value === 'FaceID' ? 'Use Face ID to unlock' : 'Use your fingerprint to unlock',
    }).then(authenticated => {
        if (authenticated) {
            const router = props.f7router;
            doAfterUnlocked();
